...
#include<bits/stdc++.h>
int getAns(vector<int> Arr, int ind, int buy, int n, vector<vector<int>> &dp,int fee ){
    ...
    if(buy==1){// We can sell the stock
        profit = max(0+getAns(Arr,ind+1,1,n,dp,fee), Arr[ind]+getAns(Arr,ind+1,0,n,dp,fee)-fee);
    }
